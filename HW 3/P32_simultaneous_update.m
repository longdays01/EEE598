...
function [iterations, potentials, player_distribution] = best_response_dynamics(M)
    ...
    while ~converged && iter < 6 % Set a limit on the number of iterations
        iter = iter + 1;
        ...
        for i = 1:M
            ...
            for temp = 1:4
                ...
                if temp ~= current_best_action
                    ...
                    % Debug output to trace changes
                    % fprintf('Player %d: Trying action %d from action %d, New Delay: %f, Current Delay: %f\n', ...
                    %         i, temp, current_best_action, delay_array_new(temp), delay_array(current_best_action));
        
                    if delay_array_new(temp) < delay_array(current_best_action)
                        new_action_matrix_update(i) = temp;
                        fprintf('Player %d: Changed action %d to %d at iter %d\n', i, current_best_action, temp, iter);
