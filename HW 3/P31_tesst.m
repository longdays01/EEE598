function P31()
    Ms = [200]; % Total number of players for each setting
...
function [iterations, potentials, player_distribution] = best_response_dynamics(M)
    ...
    while ~converged && iter < 1000 % Set a limit on the number of iterations
        ...
        new_action_matrix_update = action_matrix_update;
        for i = 1:M
            best_action = action_matrix_update(i);
            best_delay = inf;

            for temp = 1:4
                if temp ~= action_matrix_update(i)
                    ...
                    [new_delay_SAAD, new_delay_SBBD, new_delay_SAABBD, new_delay_SD, new_potential] = cal_delay(new_players_SAAD, new_players_SBBD, new_players_SAABBD, new_players_SD);
                    new_delays = [new_delay_SAAD, new_delay_SBBD, new_delay_SAABBD, new_delay_SD];

                    if new_delays(temp) < best_delay
                        best_delay = new_delays(temp);
                        best_action = temp;
                    end
                end
            end
            new_action_matrix_update(i) = best_action;
        end

        
        action_matrix_update = new_action_matrix_update;
